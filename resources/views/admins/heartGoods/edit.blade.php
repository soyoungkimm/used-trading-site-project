...
<main>
    <div class="container-fluid px-4">
        ...
        <div class="card mb-4">
            ...
            <form name="form1" action="/admin/heart-goods/{{ $heart_good->id }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                                    ...
                                    <select class="form-select @error('goods_id') is-invalid @enderror" name="goods_id" required>
                                        ...
                                        @if (old('goods_id') != '') 
                                            @foreach ($goods as $good)
                                                @if ($heart_good->goods_id == $good->id) 
                                    ...
                                    <select class="form-select @error('user_id') is-invalid @enderror" name="user_id" required>
                                        ...
                                        @if (old('user_id') != '') 
                                            @foreach ($users as $user)
                                                @if ($heart_good->user_id == $users->id) 
